...
class TorchSE3Control(object):
    ...
    def __init__(self, yaml_path: str, device):
        ...
        rotor_pos_np = np.array([cfg["geometry"]["rotor_pos"][k] for k in sorted(cfg["geometry"]["rotor_pos"].keys())])
        ...
        f_to_TM = np.vstack(
            (
                np.ones((1, self.num_rotors)),
                np.hstack([np.cross(r, np.array([0, 0, 1]))[:2].reshape(-1, 1) for r in rotor_pos_np]),
        ...
        self.TM_to_f = torch.from_numpy(TM_to_f_np).to(device=self.device, dtype=torch.float32)
    ...
    def _batch_safe_hopf(self, zeta, zeta_dot, yaw, yaw_dot, eps=1e-6):
        ...
        """
        # Ensure shapes
        if yaw.dim() == 1: yaw = yaw.unsqueeze(1)         # (B, 1)
        if yaw_dot.dim() == 1: yaw_dot = yaw_dot.unsqueeze(1)
        ...
        q_psi = torch.stack([torch.cos(half_yaw), zeros, zeros, torch.sin(half_yaw)], dim=1)
        
        # ---- Total Quaternion ----
        q_tot = self.quat_mul(q_abc, q_psi) # [w, x, y, z]
        
        # Output q in Scipy order [x, y, z, w] for compatibility if needed
        q_scipy = torch.stack([q_tot[:, 1], q_tot[:, 2], q_tot[:, 3], q_tot[:, 0]], dim=1)
    ...
    def update(self, t, state, flat , omega_cmd=None):
        ...
        motor_speeds = torch.sign(motor_speeds_sq) * torch.sqrt(torch.abs(motor_speeds_sq))
